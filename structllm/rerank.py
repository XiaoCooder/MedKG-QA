...
def rerank(args, question):
    ...
    results = sllm.retrieve.get_context_collection_and_query(args.encoder_model, query_texts = question , recall_num = 5)
    contexts= [candidate_question for candidate_question in results['documents'][0]]
    chunk_ids = [candidate_content.get('chunk_id', '') for candidate_content in results['metadatas'][0]]
    flag = True
    while (flag) :
        ########2.context_rerank#######
        query_prompt = sllm.query_prompt.query_prompt(args, contexts)
    ...
    results = sllm.retrieve.get_qas_collection_and_query(args.encoder_model, query_texts = question , recall_num = 5)
    questions = [candidate_question for candidate_question in results['documents'][0]]    
    chunk_ids = [candidate_content.get('chunk_id', '') for candidate_content in results['metadatas'][0]]
    flag = True
    while (flag) :
        ########3.qa rerank#######
