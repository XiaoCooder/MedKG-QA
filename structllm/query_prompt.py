...
class query_prompt():
         ...
         try:
           with open(args.reranker_prompt,'r',encoding = 'utf-8') as json_file:
                ...
                self.naive_prompt.append(
                         {
                             "role": "user",
                             "content": self.add_ask_Prompt(self.data, question)
                         }
                )
         except json.JSONDecodeError as e:
             #print(f"JSON 解码错误: {e}")
    ...
    def add_ask_Prompt(self, data, question, task):
        ...
        if task == "qas_rerank":
           Prompt = "the question is:" + question
           task_prompt = "\nThe following are several question and answer pairs. \nSort the pairs that are most likely to have an answer based on the question.\nreturn an array as format, and put the most possible answer first, for example: [0, 2, 1]"   
           Prompt = Prompt + task_prompt
           for i in range(len(data)):
              Prompt = Prompt + f"\nparagraph{i}" + data[i]
